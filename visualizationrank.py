    ...
    if sort_by == "none":        
        for item in data.items():
            if type(item[1][next(iter(item[1]))]) == tuple:
                d[item[0]] = {k: v for k, v in item[1].items()}
            else:
                d[item[0]] = {k: v for k, v in item[1].items()} 
        ...
        if label_shortening == "resize":
            while len(h)*(l/2) > n2+points[0].size-(n2/8) and l > 1:
                if x != max(h_x_shift):
                    ...
                    break
            d.append(draw.Text(h, x=x, y=height - spacing+2, fontSize=l, font_family=label_font, fill=label_color))
        elif label_shortening == "clip":
            clip = draw.ClipPath()
            clip.append(draw.Rectangle(x, height - spacing, n2, label_size))
            d.append(draw.Text(h, x=x, y=height - spacing+2, fontSize=l, font_family=label_font, clip_path=clip, fill=label_color))
            ...
            if len(h)*(label_size/2) > n2+points[0].size-(n2/8):
                ...
                txt = h
            d.append(draw.Text(txt, x=x, y=height-spacing+2, fontSize=l, font_family=label_font, fill=label_color))
    ...
    for n in sequence.items():        
        if len(n[1]) > 1:
            for k in n[1][:-1]:
                ...
                if points[k].category != category_noplot:
...
def visualizeRank(data, spacing=50, node_size=10, width=None, height=None, minValue=1, maxValue=10, node_scaling="linear",
              connection_type="semi-curved", color_startEnd=True, color_categories=True, nodes_color="gray",
              ...
              line_stroke_width=0.5, line_stroke_thick=0.5, legend=True, sort_by="frequency", category_noplot = "No outbreak"):
    """
    Generates an SVG from data loaded via the read functions.
    ...
    data (dict): output of the read_file/read functions, a dictionary with keys the temporal data, and values a dictionary with keys of the item and values or the frequency of the item or a tuple with the frequency and the category
    spacing (int): the space between the nodes, defaults to 50
    node_size (int): default node size, defaults to 10
    width (int): width of the visualization, defaults to None, if None they are generated from the size of the nodes, if they are specified the nodes will be rescaled to fit the space
    height (int): height of the visualization, defaults to None, if None they are generated from the size of the nodes, if they are specified the nodes will be rescaled to fit the space
    minValue (int): min size of a node , defaults to 1
    maxValue (int): max size of a node, defaults to 10
    ...
    nodes = nodify(data, sort_by=sort_by)
    viz = genSVG(nodes, spacing, node_size, width=width, height=height, minValue=minValue,
                 ...
                 line_stroke_width=line_stroke_width, line_stroke_thick=line_stroke_thick, legend=legend,category_noplot=category_noplot)
    return viz
